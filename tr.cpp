...
ArrayHeap* AH_newHeap(const int max) {
	ArrayHeap* pheap;
	if ((pheap = (ArrayHeap*)malloc(sizeof(ArrayHeap))) == NULL) exit(1);
	if ((pheap->nodes = (HNode*)calloc(max + 1, sizeof(HNode))) == NULL) exit(1);
...
Graph* GR_newGraph(const int vtxsize, const int edgesize) {
	Graph* pgraph;
	if ((pgraph = (Graph*)malloc(sizeof(Graph))) == NULL) exit(1);
	if ((pgraph->tails = (GNode**)calloc(vtxsize, sizeof(GNode*))) == NULL) exit(1);
	if ((pgraph->_edges = (GNode*)calloc(edgesize + vtxsize, sizeof(GNode))) == NULL) exit(1);
...
inline void GR_addEdge(Graph* pgraph, const int from, const Edge edge) {
...
int cmp(void* a, void* b) {
	struct EdgeStack* A = (EdgeStack*)a, * B = (EdgeStack*)b;
	return A->from - B->from;
}
int main() {
	int* distances;
	int V, E, start, stack_len;
	Graph* graph;
	struct EdgeStack* stack;

	scanf("%d%d%d", &V, &E, &start);
	graph = GR_newGraph(V + 1, E);
	stack = (EdgeStack*)calloc(E, sizeof(struct EdgeStack));
	stack_len = 0;
	for (int i = 0; i < E; i++) {
		scanf("%d%d%d", &stack[i].from, &stack[i].edge.id, &stack[i].edge.weight);
	}
	// qsort(stack, E, sizeof(struct EdgeStack), cmp);
	for (int i = 0; i < E; i++) {
		GR_addEdge(graph, stack[i].from, stack[i].edge);
	}
	free(stack);
